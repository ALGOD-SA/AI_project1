...
def depthFirstSearch(problem, depth_limit=100):
    """Search the deepest nodes in the search tree first with cycle detection and depth limit."""

    # States to be explored (LIFO). Holds nodes in the form (state, action)
    frontier = util.Stack()
    # Previously explored states (for path checking). Holds states
    exploredNodes = set()  # Using a set for faster membership checking
    ...
    while not frontier.isEmpty():
        ...
        if currentState not in exploredNodes:
            ...
            if problem.isGoalState(currentState):
                return actions, max_depth, expanded_nodes_count, max_fringe_size
            else:
                # Get a list of possible successor nodes in the form (successor, action, stepCost)
                successors = problem.getSuccessors(currentState)

                # Push each successor to the frontier
                for succState, succAction, _ in successors:
                    newAction = actions + [succAction]
                    newNode = (succState, newAction)
                    frontier.push(newNode)

    return [], max_depth, expanded_nodes_count, max_fringe_size  # No solution found within depth limit
...
def breadthFirstSearch(problem):
    ...
    while not frontier.isEmpty():
        ...
        if currentState not in exploredNodes:
            ...
            if problem.isGoalState(currentState):
                return actions, len(actions), expanded_nodes_count, max_fringe_size
            else:
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not frontier.isEmpty():
        ...
        if problem.isGoalState(currentState):
            depth = len(actions)
            return actions, depth, expanded_nodes_count,max_fringe_size
